...
class QMC5883:
    def __init__(self,i2c,irq=False,slvAddr=0x0D,autoScale=True):
        ...
        """if(irq):
            ...
            self.ctrlR2=b'\x41'"""
        if(~autoScale):
            self.i2c_writereg(0x09,b'\x19')#OSR=00(512),range=00(00:2Gauss/01:8Gauss),ODR=10(10:100Hz 11:200Hz),MODE=01(start continuous measure)
        else:
    ...
    def measure(self):
        xyz=[0,0,0]
        mag=bytearray(6)
        if(~self.autoScale):#8gauss range, doesn't consider over range situation
            mag=self.i2c_readregs(0x00,6)
        else:
            self.status=self.i2c_readregs(0x06,1)
            if(status[0]&0x02==0x00):#not over range
                mag=self.i2c_readregs(0x00,6)
            else:
